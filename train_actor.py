...
from src.buffers.off_policy import OffPolicyBuffer
...
class Trainer:
    def __init__(self, config: DictConfig):
        ...
        state_vec_dim = 4
        state_dim = state_z_dim + state_vec_dim
        action_dim = 3
        self.agent = TD3Agent(state_dim=state_dim, action_dim=action_dim, device=self.device, **config.agent)

        self.buffer = OffPolicyBuffer(state_z_dim=state_z_dim, state_vec_dim=state_vec_dim, action_dim=action_dim, **config.buffer)

        vae_cfg = config.vae
        cnn_cfg = vae_cfg.cnn
        self.vae = CNN_VAE(cnn_cfg, vae_cfg.latent_dim, vae_cfg.input_shape).to(self.device)
            ...
            for episode in range(self.max_episodes):
                ...
                for step in range(self.max_steps):
                    ...
                    with Timer("Env Encoding"):
                        ...
                        mu, logvar = self.vae.encode(obs_img)
                        state = self.vae.latent(mu, logvar)
                    ...
                    with Timer("Next Encoding"):
                        ...
                        mu, logvar = self.vae.encode(next_obs_img)
                        next_state = self.vae.latent(mu, logvar)
